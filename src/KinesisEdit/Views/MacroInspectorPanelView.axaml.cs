...
namespace KinesisEdit.Views
{
    ...
    /// half of the step list's reorder — nothing else. It used to carry a second handler, the macro
    /// name field's Enter commit, and that went with the field itself in issue #146: the panel has
    /// no name to type any more, so the one thing a binding could not spell has nothing to spell it
    /// for. The drag's keyboard half is the editor's own grammar
