...
- **`Suspend`/`Resume` have two consumers.** Every feature panel that is *not* a keystroke sink — Macro Timing Delays, Search Keys, Export — is opened with an explicit `Suspend()` and closed with `Resume()`, which is spec 10's "capture is suspended whenever a dialog that needs real typing is open"; and the pedal editor wraps every message box it raises — the mid-edit "apply changes?", the unsaved-changes question, the save failure — in the same pair, because a modal window is still inside the `TopLevel` the adapter previews, so an unsuspended capture would swallow the dialog's own Enter and Escape. The keyboard editor raises its boxes only when nothing is listening. The adapter's auto-suspend on `TextBox` focus still applies, but it is a platform judgement; the explicit pair is the deterministic guarantee — it is why those panels can be typed into while the editor behind them is mid-edit, and why the pedal wraps *all* of its boxes rather than only the ones raised mid-edit (suspending a stopped capture is a no-op).
- **The editor's own Escape is a casualty of "swallow everything", by design.** While a key listens — or a Tap and Hold field is armed — Escape resolves like any other position and becomes the assignment (a keyboard must be able to carry an Escape remap), so dismissal of *those* is a pointer action. An **open feature panel is the exception**: the editor dismisses it on Escape regardless of whether capture consumed the event, because capture may be running for something else and a panel that cannot be closed from the keyboard is worse. See [keyboard-editor.md](keyboard-editor.md) for the full rule and the tunnel-order reason its Escape handler is a safety net rather than the path. **The pedal editor has no Escape handler at all**: its entry box does not leave the recording state on a keystroke, so an Escape reaching such a handler would be recorded *and* discard the entry — it records Escape like any other key and cancels through its Cancel button.
