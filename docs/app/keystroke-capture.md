...
## The consumers — the keyboard editor and its panels

`KeyboardEditorViewModel` owns the **single** subscription to `KeystrokeCaptured` for the editor's lifetime and routes each keystroke to exactly one target ([keyboard-editor.md](keyboard-editor.md), "Keystroke routing"): an **open** feature panel that is an `IKeystrokeSink` → a recording macro → the listening key. The sink wins on being open, not on having a field armed — spec 10 forwards a captured key "to the Tap and Hold dialog if that dialog is open". One capture service instance is built lazily by `App` over the shell window and shared by every editor; the shell disposes it after the editor.

- **Two things start capture, never both at once.** Macro recording (the panel raises `RecordingChanged`; the *editor* calls `Start`/`Stop`) and an **armed Tap and Hold field** (`EditorOverlayHost` follows the panel's `IKeystrokeSink.WantsKeystrokes`) — plus the remap's listening key, which entering either of the other two cancels first. Opening any feature panel ends *both* of the others (`KeyboardEditorViewModel.ShowOverlay` cancels listening and stops recording), so the host always finds the service free and its start/stop bookkeeping is never bypassed.
...
- **`Suspend`/`Resume` now have a real consumer.** Every feature panel that is *not* a keystroke sink — Macro Timing Delays, Search Keys, Export — is opened with an explicit `Suspend()` and closed with `Resume()`, which is spec 10's "capture is suspended whenever a dialog that needs real typing is open". The adapter's auto-suspend on `TextBox` focus still applies, but it is a platform judgement; the explicit pair is the deterministic guarantee, and it is why those panels can be typed into while the editor behind them is mid-edit.
- **The editor's own Escape is a casualty of "swallow everything", by design.** While a key listens — or a Tap and Hold field is armed — Escape resolves like any other position and becomes the assignment, so dismissal of *those* is a pointer action. An **open feature panel is the exception**: the editor dismisses it on Escape regardless of whether capture consumed the event, because capture may be running for something else and a panel that cannot be closed from the keyboard is worse. See [keyboard-editor.md](keyboard-editor.md) for the full rule.
