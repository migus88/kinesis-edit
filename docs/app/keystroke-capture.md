...
## The consumers — the keyboard editor and the pedal editor

`KeyboardEditorViewModel` is the first consumer ([keyboard-editor.md](keyboard-editor.md)): it subscribes to `KeystrokeCaptured` for the editor's lifetime, calls `Start()` **only** when a key enters listening state, applies the captured `CapturedKeystroke.Key` through `KeyboardKey.Remap`, and calls `Stop()` on the keystroke, on cancel, on a layer switch and on `Dispose`. `SavantElitePedalViewModel` is the second ([savant-elite.md](savant-elite.md)), with the same lifetime shape: `Start()` when an input's entry box opens, every captured keystroke appended to (or, in single mode, replacing) the entry, `Stop()` on Done, Cancel and `Dispose`. One instance is built lazily by `App` over the shell window and shared by every editor; the shell disposes it after the editor.

- **The editor's own Escape is a casualty of "swallow everything", by design.** While a key listens, Escape resolves like any other position and becomes the assignment — a keyboard must be able to carry an Escape remap — so the editor cancels by pointer instead. See [keyboard-editor.md](keyboard-editor.md) for the full rule and the tunnel-order reason its Escape handler is a safety net rather than the path. **The pedal editor has no Escape handler at all**: its entry box does not leave the recording state on a keystroke, so an Escape reaching such a handler would be recorded *and* discard the entry — it records Escape like any other key and cancels through its Cancel button.
- **`Suspend`/`Resume` has one consumer: the pedal editor's message boxes.** Every box it raises — the mid-edit "apply changes?", the unsaved-changes question, the save failure — is wrapped in `Suspend()`/`Resume()`, because a modal window is still inside the `TopLevel` the adapter previews, so an unsuspended capture would swallow the dialog's own Enter and Escape. The keyboard editor raises its boxes only when nothing is listening, and the adapter's text-input auto-suspend covers focus landing in a `TextBox`; neither replaces the explicit calls. Suspending a stopped capture is a no-op, which is why the pedal wraps *all* of its boxes rather than only the ones raised mid-edit.
