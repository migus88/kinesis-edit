# Feature dialogs (Macro Delays, Search Keys for macros, Export, Import)

The advanced-feature surfaces of spec 11 that hang off the keyboard editor: **Macro Timing Delays** (§11.3), **Search Keys** (§11.6) where it is still a modal, **Export files** (§11.5), and **Import** (spec 10 + 07 §1.4, which has no dialog of its own).

**This module lost two of its five panels to the key inspector rail (issue #92), and it is smaller for it.** §11.1's *Assign Tap and Hold Action* is now `TapAndHoldPanelViewModel`, a mode of the rail; §11.6's plain picker is `RemapPanelViewModel` + `TokenPickerViewModel`, hosted inline beside the board. Both are documented in [keyboard-editor.md](keyboard-editor.md) § "The key inspector rail". **Their Core-side rules did not move**: `TapAndHoldPrecheck`/`TapAndHoldRefusal` and `KeySearchCatalog` are still where they were, still UI-free, and are still described below — a rail panel enforces exactly the rules the modal did. What is gone is the modal, its `Ok`/`Cancel` pair, and the *nesting* the two `Search` buttons needed: `EditorOverlayHost.ShowNested` was removed with its only consumer rather than left as tested dead code.

The three that remain are genuinely modal — each asks a question whose one answer has to come back to the caller — and §11.6's catalog is still one of them, as `Search Keys (Macro)`. The macro panel they feed is part of the editor ([keyboard-editor.md](keyboard-editor.md)); the rules they enforce are Core's — the four pre-dialog checks, the delay tokens, the search catalog, the import heuristic and the export plan all live in `KinesisEdit.Core` so the wording and the validation are testable apart from any window.
...
| `KinesisEdit.Views` | `MacroDelayOverlayView`, `TokenPickerOverlayView`, `ExportOverlayView` | XAML only; `ViewLocator` resolves them from the open panel | 11 |
...
It lives beside the panels rather than in `Services/` because it is presentation: Core carries the gate rows, their messages and the support URLs as data and deliberately shows no dialog ([firmware.md](firmware.md)). Each gated feature exposes a one-line static wrapper (`TapAndHoldPanelViewModel.EnsureFirmwareAvailableAsync`, `MacroDelayOverlayViewModel.EnsureFirmwareAvailableAsync`) so a caller never maps a feature onto a `FirmwareFeature` itself. **§11.1 no longer calls its wrapper from the editor**: the rail's panel asks `FirmwareGateService.IsAvailable` on every refresh and answers in place through `IsAvailable`/`UnavailableReason`, with an `Update Firmware` button of its own — a modal refusal for a surface that is already on screen would be an interruption with nothing to interrupt. The fallback messages exist because 09 §2 quotes the refusal only under the Freestyle rows while §11.1 describes one for every gated app — tests pin each fallback identical to the row that does store it.
...
0. **Two of the five panels are no longer panels at all.** §11.1 and §11.6's plain picker are key inspector modes (issue #92). The rules, the refusals and the catalog are unchanged; the modal, its `Ok`/`Cancel` pair and the nesting are gone. Recorded here because this doc's own scope changed.
