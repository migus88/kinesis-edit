...
## §11.3 — Macro Timing Delays (**now the key inspector's step editor**)

The surface moved onto the step with issue #93 ([keyboard-editor.md](keyboard-editor.md)); **every rule below is unchanged** and is what `MacroInspectorStepsViewModel` enforces. Core answers *what a delay is*; the panel answers *which step carries it*.
...
The editor exposes `IsRandomDelay` and `CustomDelayMilliseconds` (0 = empty; **assigning any value un-checks the random radio**), Up/Down commands clamped to 1–999, and `ApplyDelayCommand`, which writes the resolved `KeyDefinition` onto the row it is open over. **Neither control is preselected**, so an untouched editor fails validation — §11.3's outcome is "random, 1..999, or cancel/invalid". A value outside the range *and* a dialect that does not name the token are indistinguishable and report the same message.

What the rail changed, and only this: the delay is written **beside the step it belongs to** rather than appended to the end of the recording, `ClearDelayCommand` takes one off again (which the modal had no way to do at all), and the firmware gate is answered in place. The millisecond field **is** a real `TextBox` — the opposite of the action-field rule and correct, because a delay is typed and suspending capture is what should happen.
