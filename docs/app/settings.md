...
8. **`app_settings.txt` has one reader and one writer** — this module, reached through `ISettingsService`, and in the app layer through the session's single `IAppPreferencesStore`. Anything that wants a flag, a preference or a colour reads `Current` and writes through `Update` (a macro *name* through `UpdateMacroNames`, which is the same one writer with the profile scope Core's writer needs); a second reader shows stale state and a second line-level implementation drops whatever it does not model.
