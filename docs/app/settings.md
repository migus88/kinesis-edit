...
| `KinesisEdit.Core.Settings` | `AppSettings`, `SettingsColor`, `AppSettingsParser`, `AppSettingsSerializer` | Sixteen hide flags + one display preference + custom colors + macro names | 08 §3 (+ 5 app-added keys + the `macro_name_*` family) |
| `KinesisEdit.Core.Settings` | `MacroNameKey` | Identity of one macro name on disk: profile + layer + trigger + slot | — (app-added; 06 §1, 05 §1.3) |
...
- `FindPrefixedValues(lines, keyPrefix)` — every line whose key starts with the prefix, as (key remainder, value) pairs **in file order**, duplicates kept so a caller applying them in sequence lands on the same last-one-wins answer.
...
`FindPrefixedValues` is the same rule applied to a key family whose full names are not known in advance — today only `macro_name_*` (below). The key is everything before the **first** `=` after the prefix, and a line with nothing between the prefix and the separator does not qualify, so the scan can never claim a foreign line.
...
### Macro names — the `macro_name_*` family

`macro_name_<profile>_<layer>_<trigger>_<slot>` = the macro's name. **Not a spec 08 key** — this app adds the whole family, safe on disk for the same reason as the five above. All four components are unsigned invariant decimals; `<slot>` is 1–5 on a slot device and **0** for an Advantage360 flat-list macro (06 §1).

- `MacroNameKey` (`readonly record struct`) — `ProfileNumber` (≥ 1), `LayerIndex`, `TriggerKeyCode`, `SlotNumber`; `TryCreate(...)`, `TryParse(settingsKey)`, `TryParseSuffix(suffix)`, `ToSettingsKey()`. `SettingsKeys.MacroNamePrefix` = `macro_name_`, `SettingsKeys.GetMacroNameKey(key)` builds it.
- `AppSettings.MacroNames` (`IReadOnlyDictionary<MacroNameKey, string>`), `GetMacroName(key)`, `WithMacroName(key, name)`, `WithMacroNamesForProfile(profileNumber, names)`.
- `AppSettingsParser` reads the family by prefix scan; `AppSettingsSerializer.SerializeMacroNames(settings, profileNumber)` writes it and `SerializeMacroNameRemovals(settings, profileNumber)` deletes the stale keys; `SettingsService.SaveMacroNames(location, settings, profileNumber)` puts both through the merge.

**Why the place and not `Macro.Id`.** `Macro.Id` is a fresh `Guid` per edit and is never written to any file, so it cannot survive a reload. The macro's *place* is what both files can name (05 §1.3, 06 §1) — and the trigger component is `KeyboardKey.TriggerKey`, not the position key, or `fn1s`/`keyt` would be written under a key the next load never looks up. A macro with no trigger or layer (an unbound Gen2 flat-list macro) has no key at all: `TryCreate` returns false and its name is not persisted.

**The profile is part of the key, and part of the API.** One `app_settings.txt` holds every profile's names, so saving profile 1 must never remove profile 2's. Every macro-name method takes the profile number, and `Serialize`/`SerializeRemovals` — the swatch and preference path, which has no profile — deliberately **cannot reach a name at all**, neither to write nor to delete. That is the mistake made structurally impossible rather than merely documented.

**An empty value is a tombstone, not a name.** A blank name is never written (unnamed macros derive a display name from their content, which follows the content instead of freezing), so "skipped" and "cleared" would look identical to a merge — invariant 5's trap, exactly as it bit the colour swatches in #95. An entry whose value is empty therefore means *delete this key*: `SerializeMacroNames` skips it and `SerializeMacroNameRemovals` emits it. `WithMacroNamesForProfile` is what makes tombstones: hand it the profile's complete current picture (`MacroSites.EnumerateStoredNames(layout)` — one pair per site whose `Macro.Name` is non-empty — mapped through `MacroNameKey.TryCreate`) and every rename, unassignment and delete resolves by itself — whatever is gone becomes a removal, and other profiles are untouched. Passing it a key from another profile throws.

> **Deliberate exception to "write through immediately".** Every other consumer of this file writes the moment the user answers — a swatch, a "don't ask again" tick. **A macro name does not: it is part of the editor session's dirty model and reaches the drive with the profile Save**, because a rename belongs to the same edit as the macro it names and must be discardable with it. That is why it has its own service method and its own serializer pair rather than riding `SaveAppSettings`.
...
void UpdateMacroNames(int profileNumber, Func<AppSettings, AppSettings> mutate);   // issue #93
bool IsHidden(string key);          // inherited from INotificationSuppressionStore
void SetHidden(string key, bool hidden);
```

**`UpdateMacroNames` is a second write path, not a flag on the first**, because Core's writer is a second entry point too: `SaveMacroNames` takes the profile number that scopes the deletion — one file holds nine profiles' names — and `SaveAppSettings` cannot reach a name at all (invariant 11). It is also the *only* member driven by the editor's Save rather than by the edit itself; see the "deliberate exception" note above. The read-only and null stores discard it exactly as they discard `Update`, so demo mode keeps its names in memory and throws nothing.
...
- `SaveMacroNames(VDriveLocation, AppSettings, int profileNumber)` — the macro-name half alone, scoped to one profile (above). Same merge, same create-on-missing behaviour; it touches no flag, no colour and no other profile's names.
...
8. **`app_settings.txt` has one reader and one writer** — this module, reached through `ISettingsService`, and in the app layer through the session's single `IAppPreferencesStore`. Anything that wants a flag, a preference or a colour reads `Current` and writes through `Update` (a macro *name* through `UpdateMacroNames`, which is the same one writer with the profile scope Core's writer needs); a second reader shows stale state and a second line-level implementation drops whatever it does not model. **Its sibling rule**: the *host* preferences (theme, motion, window geometry) are a different file with a different scope and a store of their own — `IHostPreferencesStore` ([host-preferences.md](host-preferences.md)) — and neither store may grow into the other. A preference about a keyboard goes on the drive; a preference about the person or the machine does not.
...
11. **A macro-name removal is scoped to the profile being written.** One `app_settings.txt` holds every profile's names, so `SerializeMacroNames`/`SerializeMacroNameRemovals`/`SaveMacroNames` all *require* the profile number, and `Serialize`/`SerializeRemovals` — which have no profile — cannot touch a name at all, to write or to delete. A blank name is a tombstone (delete the key), never an empty value on disk, and `WithMacroNamesForProfile` is the only thing that makes tombstones — from the profile's complete current picture, so nothing else can go stale. Names are also the one part of this file **not** written through immediately: they belong to the editor session's dirty model and reach the drive with the profile Save.
