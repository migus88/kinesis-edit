...
- `Profiles` — load/save orchestration for numbered-profile devices (FS Edge/Pro, Edge RGB, TKO, Advantage360): `ProfileSession` (load, `IsDirty`, `CanSave`, `Save`/`SaveAs`), `ProfileSaveResult`, `ProfileReadOnlyException` (the Adv360 profile-0 guard), `ProfileSaveMessageCatalog`, `ProfileLightingCodec` (spec 03 §4.1/§4.3, §5.3; 04 §5.3; 07 §1.1-§1.3) — see [`profiles.md`](profiles.md).
