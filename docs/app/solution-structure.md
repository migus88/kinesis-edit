...
- `Services` — detection loop (`DeviceMonitorService` → `DeviceSnapshot`), notifications (message box, toast, loading, `Hide this notification?` suppression), the active-device session, eject, URL launching, UI-thread dispatch, and the editor's native folder/file pickers with the profile importer behind them (see [`feature-dialogs.md`](feature-dialogs.md)). Toolkit-free apart from the bridging services `AvaloniaUiDispatcher`, `MessageBoxPresenter`, `AvaloniaFolderPickerService`, `AvaloniaFilePickerService` and their `DialogWindowHost`/`StorageProviderHost` helpers.
- `ViewModels` — `MainWindowViewModel` (navigation, Home → eject, status indicator), `DashboardViewModel`/`DeviceCardViewModel`/`NoDeviceViewModel`, the editors hanging off `DeviceEditorViewModel` (`EditorPlaceholderViewModel`, the keyboard editor `KeyboardEditorViewModel` with its layer/key view models — see [`keyboard-editor.md`](keyboard-editor.md) — and the pedal editor `SavantElitePedalViewModel` with its row and Special Actions view models — see [`savant-elite.md`](savant-elite.md)), plus the notification view models. They expose enums and strings, never Avalonia brushes.
- `Views` — the `.axaml` for each of the above plus `MainWindow`, `MessageBoxWindow`, `NotificationOverlay`. Resolved by name through `ViewLocator`. Also here: `KeystrokeCaptureSpikeWindow`, the issue-#12 capture harness, which is not part of the shipped UI — `App` opens it as the main window instead of the shell only when started with `--keystroke-spike` (see [`keystroke-capture.md`](keystroke-capture.md)).
- `Themes`, `Styles`, `Assets/Fonts` — the design system (see [`design-system.md`](design-system.md)). `Themes/` holds the tokens (`Tokens.axaml` colour roles in `Dark`/`Light` theme dictionaries, `Geometry.axaml` shape, `Typography.axaml` families + type scale, `Motion.axaml` the motion budget, `Controls.axaml` the FluentTheme alias map); `Styles/` the shared style layer split by concern (`Text`, `Surfaces`, `Buttons`, `Keyboard`, `Editor`); `Assets/Fonts/` the embedded IBM Plex Sans/Mono TTFs plus their SIL OFL licence, shipped as `AvaloniaResource`. `App.axaml` merges them and defines nothing itself — **no colour, size or style may be added to it**. `Themes/Controls.axaml` must stay merged into `Application.Resources`, never `Application.Styles`, or FluentTheme's own palette wins.
...
- `Converters` — `EnumMatchConverter`, `MessageBoxIconToGlyphConverter`, `MenuItemToggleTypeConverter`, `HexColorToBrushConverter`, `HexColorToColorConverter`: the enum → style-class/glyph/toggle-type and `#RRGGBB` → brush/`Color` mapping that keeps colors and control-theme detail in XAML.
