...
- `Services` — detection loop (`DeviceMonitorService` → `DeviceSnapshot`), notifications (message box, toast, loading, `Hide this notification?` suppression), the active-device session, eject, URL launching, and UI-thread dispatch. Toolkit-free apart from the two bridging services `AvaloniaUiDispatcher` and `MessageBoxPresenter`.
