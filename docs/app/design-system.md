...
| `Themes/Icons.axaml` | `ResourceDictionary` | 13 state/action/chrome marks as `StreamGeometry`. Colourless, theme-independent. |
...
| `PaddingInspectorSection` | `12` | Inspector sections, and `Border.keyboardBoard` — the Lighting tab's board surround. The Layout canvas's own panels use `PaddingBoardSection` below |
...
|---|---|---|---|
| `Themes/Icons.axaml` | 16 | `Icon<Name>` | Nothing — the set is fixed and listed in `IconCatalogTests` |
...
| `Keyboard.axaml` | `Border.keyboardBoard` (the Lighting tab's surround only — the Layout canvas draws its own section panels), `Button.keyCap` **(bridge → `KeyCapButton`)**, `.keyCapText` (+`.locked`), `.keyCapSubText`, the recording pulse |
| `Editor.axaml` | `Border.actionField` **(bridge → `TokenField`)**, `Button.modeOption` **(bridge → `ModeOption`)**, `Button.colorSlot` **(bridge → `SecondaryButton`)** (+`.selected`, which adds the accent ring), `Icon.modeMark` (+`.filled`, +`.selected`), `Button.colorSwatch`, `Border.colorSwatchFill`, `Border.inspectorRail` (+`.wide`), `Border.inspectorSection`/`.inspectorFooter`/`.inspectorKeyTile`, `Ellipse.recordDot`, `Button.macroStepRow` **(bridge → `RowButton`)**, `Border.macroDelayEditor`, `Icon.dragHandle`, `Icon.insertMark`, `Border.macroSlotCard` (+`.selected`) |
...
| `KeyCapButton` | `Button` | `BaseButton` | `.locked` (+ `Rectangle#Locked`) / `:pointerover` / `.selected` (+ `Border#Ring`) / `.listening`; and the four badges, each `.stateBadges` + its own class → its own part: `.modified` → `Border#RemapBar`, `.macro` → `Ellipse#MacroDot`, `.tapHold` → `Path#TapHoldMark`, `.advisory` → `Border#AdvisoryBar`, plus `.macro.advisory` insetting the dot | `Classes="keyCap"`; the seven state classes and `Classes.stateBadges` are written by `KeyCapView` |
...
- **The stacked caption on a lighting board fits its cap exactly, with nothing to spare.** Two 8 px lines (21 px once Avalonia rounds the desired size up to the device pixel) plus the 3 px LED strip is precisely the cap's 24 px content box, which is why that strip carries no top margin of its own — it reads as a gap only because the caption's box ends ~1 px below its last line's ink. Nothing further can be added to that cap without a step down or a taller cell. The *earlier* form of this note — a cap carrying a word sub-legend **and** a strip — is closed rather than residual: the sub-legend is no longer drawn where the strip is, on the same reasoning that already withheld the state badges there. `KeycapThemeTests.NoLegendOnTheWholeBoard_IsCutShortByTheBoxItIsDrawnIn` holds the whole budget. Also in [keyboard-editor.md](keyboard-editor.md).
