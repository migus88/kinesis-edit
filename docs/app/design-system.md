...
| `WidthInspectorRail` / `WidthInspectorRailWide` | 268 / 300 | The rail landed with issue #92 and uses the first. `WidthInspectorRailWide` is still unused — it is the macro-editing variant of mockup `2i`, which is #93's |
...
|---|---|---|---|
| `Themes/Icons.axaml` | 18 | `Icon<Name>` | The 14 state/action/chrome marks are fixed and listed in `IconCatalogTests`; the 4 arrows are enumerated by `LightingDirection` in `IconCoverageTests` |
...
- **The editor's remaining actions still sit in a provisional row under the content**, smaller than it was. `Reset layer` left with issue [#91](https://github.com/migus88/kinesis-edit/issues/91) for its real home in the legend row under the board; the seven that remain — `Reset Key`, `Reset Layout`, the four spec 11 buttons and `Import` — wait for the inspector and the editor-level actions of [#92](https://github.com/migus88/kinesis-edit/issues/92)/[#93](https://github.com/migus88/kinesis-edit/issues/93). Nothing in the row is the design's shape; it exists so every action stays reachable in the meantime.
...
- **Three call sites still draw a glyph rather than a mark**, deliberately: the `▲▼` steppers in `MacroDelayOverlayView`, the `✕` in `KeyboardEditorView` (its `•` is a glyph too, but a covered one — U+2022 is in both families, so it is an icon-law deviation and not a rendering risk), and the `▾` in `SavantElitePedalView`. Their views are rewritten by later redesign issues, and drawing a mark for a control that is about to be replaced would be work thrown away twice. Those four codepoints are **the whole allowlist** of `ShapeAndTypeTokenTests.EveryAuthoredChromeString_HasAGlyphInBothEmbeddedFamilies`, and each is scoped to the one file that defers it: the same character in any other view still fails the gate, and `EveryDeferredChromeGlyph_IsStillNeededByItsView` fails once a site is rebuilt and its entry left behind. (`TapAndHoldOverlayView`'s left the list with the view itself in #92; the panel that replaced it draws its record dot as an `Ellipse` and its assign hint as an `Icon`, because **`●` U+25CF, `↵` U+21B5 and `⌘` U+2318 are in none of the seven embedded faces** — verified by parsing their `cmap` tables, the same way the keycap gap below was.)
