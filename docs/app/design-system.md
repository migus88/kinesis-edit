...
| `SelectableListRow` | `ListBoxItem` | — | `:pointerover` / `:pressed` / `:selected` (inset ring) / `:disabled` / `:focus-visible` (+ halo) | `ItemContainerTheme=` in `SearchKeysOverlayView` and `NoDeviceView` |
...
- **`FilterChip` has no bridge yet** — the last theme without a call site, and deliberate; see the registry note. Everything else is claimed: `eject`, `secondary` and `navPill` with the dashboard rebuild, `KbdChip` — plus two more `secondary` call sites — with the editor shell, `ghost` and `link` with the empty state and the toast (#89), `settingSwitch` with the Settings tab (#95), and `discard` with the unsaved-changes modal (#52).
