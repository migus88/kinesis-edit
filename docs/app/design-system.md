...
The message box and the loading card are drawn **inside the shell window** by `NotificationOverlay` ([app-shell.md](app-shell.md)), and **issue #89 authored no new style, token or motion for them**. They are composed entirely from what was already here:
...
| The buttons | `secondary` for the way out and the custom buttons, `primaryAction` for the affirmative |
...
| `Design/MessageBoxViewTests` | The box as a card | The button order leaving `1k`'s "way out first, primary affirmative last"; a second button acquiring the accent; the card not taking focus on arrival, so Enter and Escape land behind the scrim instead (there is no title bar to fall back on any more); Enter ceasing to activate the affirmative once it is renamed by outcome; the suppression row appearing on a dialog that carries no key |
...
- **One bridged button role still has no call site** (`discard` — its theme does, through `recordAction`). Deliberate; see the registry note. `FilterChip`, `SearchField`, `TokenField` and `MonoValueField` all acquired real call sites with the key inspector rail (#92).
...
- **The delay slider keeps Fluent's 50 px vertical extent** in the Tap & hold panel. Tightening it means rewriting `Grid#HorizontalTemplate`, which needs template-part names the control-theme layer refuses to hand out. Same gap as the one recorded for `Slider` below. `eject`, `secondary` and `navPill` acquired theirs with the dashboard rebuild, `KbdChip` — plus two more `secondary` call sites — with the editor shell, `ghost` and `link` with the empty state and the toast (#89), and `settingSwitch` with the Settings tab (#95).
