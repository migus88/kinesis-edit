...
| The app bar's Home pill is drawn active on the dashboard, and spec 10's Home means "leave the editor and eject" — which is unavailable there | `HomeCommand` is a **navigation** and stays runnable on the dashboard, where it does nothing | `NavPill` writes every selected setter as `.selected:not(:disabled)` (contract 5, and load-bearing: Avalonia applies `BasedOn` first, so an unqualified `.selected` would leave a dead pill wearing a live face). A `Button` takes its enablement from its `Command`, so gating Home on an open editor makes it disabled in exactly the state `IsHomeSelected` is true: the active face becomes unreachable and the dashboard draws Home identically to the two pills that are not implemented yet. The no-op lives in `GoHomeAsync`, not in the predicate |
...
| Mockup `1f` offers `Export layout to file…` on the Demo Mode bar | **Not shipped.** The bar carries its sentence and `Connect a device`, and nothing else | The two conditions are mutually exclusive: the bar is visible only in demo mode, and `CanExport` needs a session, which demo mode never opens (03 §3.5). The button would therefore be disabled 100% of the time it is on screen — and *"features a device lacks are not shown, not disabled"* is the design law. Every other CanExecute-disabled control in the editor can become live; this one could not. The mock draws it because it assumes demo mode has a layout to export. Export stays reachable from the editor's action row |
...
- **`NavPill`'s disabled face is still `BaseButton`'s inset one.** The app bar's Settings and Help are permanently `CanExecute → false` until [#96](https://github.com/migus88/kinesis-edit/issues/96), and the mock draws them as bare `TextDisabled` text where this draws a recessed face inside a hairline. The theme's own header states the reasoning for the inset face and `PillThemeTests` pins it, so flipping it is a design decision with a test row attached rather than a style tweak.
