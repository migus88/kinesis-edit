...
| `Design/ControlRenderTests` | The palette **at the glass**, plus the app bar's own chrome | A resource that resolves correctly but never reaches the control. It reads the themed template's own `Label` presenter now, and asserts the value arrives by *inheritance from the control* rather than from a state setter aimed at a presenter. Also: the demo chip going amber; and the **Home nav pill reading as a dead button on the dashboard** — the pill's selected face is `.selected:not(:disabled)`, so a Home whose command is gated on an open editor is disabled in exactly the state it is selected, and the strip renders as three identical grey pills |
...
- **`FilterChip` has no bridge yet, and one bridged button role has no call site** (`discard`). Deliberate; see the registry note. `eject`, `secondary` and `navPill` acquired theirs with the dashboard rebuild, `KbdChip` — plus two more `secondary` call sites — with the editor shell, and `ghost` and `link` with the empty state and the toast (#89).
