...
**Several themes are ready before their screen is, deliberately** — the theme landing first means the screen has something to name. Today `FilterChip` has no bridge at all (substrate for the key-inspector issue), and four more are bridged but written by no view yet: `eject`, `discard`, `ghost` and `link`, whose call sites (the dashboard card's Eject, the leave-with-unsaved modal's Discard, a toast's dismiss, "Which board do I have?") arrive with the screen rebuilds. `KbdChip` and `SecondaryButton` acquired their first call sites with the editor shell (issue #90): the layer segments' `⌥n` legends, and the editor toolbar's `Home` plus the Demo Mode bar's `Connect a device`. `ControlThemeFoundationTests` proves the bridge on a synthetic control; `ControlThemeBridgeTests` proves it on a real view, and only covers the classes a view actually writes. (`SearchField` is not among them: `SearchKeysOverlayView` writes `Classes="searchField"` today and `ControlThemeBridgeTests` asserts it, even though that panel's surrounding layout is still pre-redesign.)
...
- **`FilterChip` has no bridge yet, and four bridged button roles have no call site** (`eject`, `discard`, `ghost`, `link`). All deliberate; see the registry note. `KbdChip` and `secondary` acquired theirs with the editor shell.
