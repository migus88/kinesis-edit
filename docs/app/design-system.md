...
- **The per-anchor half of the advisory system is not drawn yet.** The strip is built and `KeyboardKeyViewModel.HasAdvisory` carries the anchor, but the 12×3 `StatusAdvisoryStrong` bar on the cap and the legend row's live `Advisory n` count belong to issue [#91](https://github.com/migus88/kinesis-edit/issues/91), which owns the whole keycap badge vocabulary and must land it in one piece.
- **The editor's resets and spec 11 actions sit in a provisional row under the content**, not in the legend row and the inspector the handoff draws — [#91](https://github.com/migus88/kinesis-edit/issues/91) and [#92](https://github.com/migus88/kinesis-edit/issues/92)/[#93](https://github.com/migus88/kinesis-edit/issues/93).
