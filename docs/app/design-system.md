...
|---|---|---|---|
| `Themes/Icons.axaml` | 17 | `Icon<Name>` | The 13 state/action/chrome marks are fixed and listed in `IconCatalogTests`; the 4 arrows are enumerated by `LightingDirection` in `IconCoverageTests` |
...
Marks are drawn at `IconSize` everywhere except a dialog's type mark, which is drawn at `IconSizeDialog` (24) — `MessageBoxView` sets it on both the cell and each `Icon`. Do not reach for an arbitrary size: a mark that is neither 16 nor 24 is a design decision that belongs in this doc first.
...
| `KbdChip` | **`ContentControl`** | — | `:disabled` only | `Classes="kbd"` — the `⌥n` legend on each layer segment of the editor's tab bar |
...
- **Three call sites still draw a glyph rather than a mark**, deliberately: the `▲▼` steppers in `MacroDelayOverlayView`, the `✕` and `•` in `KeyboardEditorView`, and the `▾` in `SavantElitePedalView`. (`TapAndHoldOverlayView`'s left the list with the view itself in #92; the panel that replaced it draws its record dot as an `Ellipse` and its assign hint as an `Icon`, because **`●` U+25CF, `↵` U+21B5 and `⌘` U+2318 are in none of the seven embedded faces** — verified by parsing their `cmap` tables, the same way the keycap gap below was.) Their views are rewritten by later redesign issues, and drawing a mark for a control that is about to be replaced would be work thrown away twice. (Keycap legends — `KeyRegistry.GlyphText`, `KeyVisual.Legend`/`SecondaryLegend`, `PedalSpecialActions` — are **not** in this list: they are domain data, not icons, and stay type forever. Which makes the embedded families' coverage a hard gate on what a board may print: IBM Plex carries no `☾` U+263E, no `☼` U+263C, no `①`–`⑧`, no `▶`/`⏮`/`⏭` and no `⌫`, and mockup 1e prints several of them. `ShapeAndTypeTokenTests.EveryAuthoredKeyLegend_HasAGlyphInBothEmbeddedFamilies` walks every authored legend of every catalog board against both families at every weight the scale uses and fails the build on a miss — with a companion test proving the probe is not vacuous. The substitutes are recorded in [keyboard-editor.md](keyboard-editor.md), deviation 31.)
