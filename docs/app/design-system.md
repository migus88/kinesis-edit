...
- **`FilterChip` has no bridge yet, and one bridged button role has no call site** (`discard`). Deliberate; see the registry note. `eject`, `secondary` and `navPill` acquired theirs with the dashboard rebuild, `KbdChip` — plus two more `secondary` call sites — with the editor shell, `ghost` and `link` with the empty state and the toast (#89), and `settingSwitch` with the Settings tab (#95).
