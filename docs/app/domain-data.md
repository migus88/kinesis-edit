...
**`UsesFlatMacroList` is the store selector, and it is the only one.** Everything that has to know where a device's macros live reads it off this record and never off a `DeviceId`: `KeyboardLayout.UsesFlatMacroList`, `KeyboardKey.UsesMacroSlots` (its negation, projected onto every position by the layer builder), `MacroLengthMetric.UsesSerializedTextLength`, and `MacroSites.Enumerate` — whose sites carry `SlotNumber` 1–5 with a `Key` on a slot device and `SlotNumber` **0** with a null `Key` on the flat-list one ([keyboard-model.md](keyboard-model.md)), read off the layout in one walk with no device branch at all. The key inspector's Macro panel picks its slot strip or its flat-list shape from the same field, with no device-specific view — the rule issue #93 wrote for the Macros tab (*"slot vs. flat comes from the capability record"*) and [#140](https://github.com/migus88/kinesis-edit/issues/140) inherited when that tab was deleted.
