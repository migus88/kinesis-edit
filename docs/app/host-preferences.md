...
**`InspectorRailWidth` is the keyboard editor's right-hand rail, in DIPs, and `null` means "as authored".** One key for both tabs' rails since [#124](https://github.com/migus88/kinesis-edit/issues/124) — see the last section. Issue [#119](https://github.com/migus88/kinesis-edit/issues/119) made the rail drag-adjustable; the width it is dragged to is a fact about this person's screen and their taste, not about any board, so it is stored here rather than in `app_settings.txt` — and it has to be right on the first frame of an editor opened over a keyboard that has never been plugged into this machine.
...
- **No composition-root wiring, no Settings screen, no window-geometry capture, no rail — *in this module*.** All four now exist; they simply are not here, because this module is the store, the file and the two appliers. Who builds them is `App.axaml.cs` (inside the desktop-lifetime branch, before any window: it reads `Current` once and runs both appliers, so the first frame already wears them), who binds to them is `SettingsScreenViewModel`, and who reads the live window's size is `MainWindow.RestoreGeometry`/`PersistGeometry` — all three described in [app-shell.md](app-shell.md). **`InspectorRailWidth`'s one reader and one writer is `InspectorRailWidthViewModel`**, which `KeyboardEditorViewModel` builds once from the optional `IHostPreferencesStore` the composition root passes through `EditorViewModelFactory`: it reads `Current` once in its constructor (nothing else writes the width, so a `Changed` subscription could only tell it what it just did) and writes through `Update` on every real change of its `Width`. It is **not** debounced and does not need to be — an unchanged width returns before it writes, which is what turns a continuous drag into one write at its commit. **One owner, two consumers, one key**: since [#124](https://github.com/migus88/kinesis-edit/issues/124) the editor hands that same instance to the Lighting tab, so the key inspector rail and the mode rail are two contents of one resizable column and a width dragged on either tab is the width the other opens at. The editor's own `InspectorRailWidth`/`EffectiveInspectorRailWidth` delegate to it and are what its bindings and tests still speak. There is deliberately **no** second preference for the second rail. The rails themselves are [keyboard-editor.md](keyboard-editor.md)'s.
