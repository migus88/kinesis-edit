...
`CopyFrom(other, KeyCopyScopes)` is a user action (05 §1.3 "Copying between key positions"), so its **key-data** half refuses a locked target outright. Otherwise it **replaces** the target's key data with the source's — tap-and-hold, multi-modifier and the remap all taken from the source, so a stale rule on the target never survives — filtered by what the target's device can hold: on a device without multi-modifier support the target ends with no code at all. It therefore upholds "one rule per position" like the other editor paths and can never manufacture a conflict; it can only carry one the *source* already had (only a tolerant load can build such a source), which `Validate()` then reports. Its **macro** half deliberately stays unchecked: a macro on a `CanAssignMacro == false` position is already visible, because `Validate()` reports `MacroOnRestrictedKey` for it, whereas an unchecked key-data copy onto a locked position would have been silent.

**The remap is copied as an *assignment*, not as a field pair.** What crosses is the source's `ModifiedOrOriginalKey` — the action it *performs*, the readout every cap caption and inspector line is drawn from — and the target's `ModifiedKey`/`IsModified` are then re-derived by `Remap`'s own two rules: equal to the target's `OriginalKey` clears the remap, anything else stores it. Copying the two fields raw (what it did before) made the copy a **no-op for an untouched source**, whose flag says "no remap": the target kept its own factory action, or — if it was remapped — silently lost it. It also left a **redundant** remap when the source's action was what the target already did. **One exemption:** a source carrying a tap-and-hold or a multi-modifier keeps the target's remap fields as copied, because that rule *is* its assignment and its resolved action is only its untouched original; storing that as a remap too would plant a second single-key rule on the position (`ConflictingSingleKeyRules` — 04 §4.3 writes one line per key). A **locked source** therefore copies its factory action across, which is the whole point of "a locked key can be a copy source, never a target".
