...
3. Write a short spec: problem, approach, acceptance criteria, affected modules, required doc updates (`docs/app/`, CLAUDE.md, README per repo policy).
...
1. Build and run tests (see CLAUDE.md for the current commands; `dotnet build` / `dotnet test` once the solution exists). Fix failures before moving on. If something cannot pass, say so plainly in the PR instead of hiding it.
2. Documentation gate: confirm the diff includes the doc updates identified in phase 3 — `docs/app/` for every touched/created module, CLAUDE.md, and README.md where applicable. If any are missing, go back and write them before opening the PR.
