...
**Current status: static domain data, the in-memory keyboard model, the firmware module (version parsing, feature gating, update check), the v-Drive services, the lighting module (model + led-file engine), the settings engine, keystroke capture, the layout-file engine (layout/macro parsers and serializers for all four dialects), profile load/save/eject orchestration for the numbered-profile devices (FS Edge/Pro, Edge RGB, TKO, Advantage360 — Advantage2's position-based naming is issue #37), and the Savant Elite2 pedal-file engine (`active/pedals.txt` parse/serialize/save) implemented; the app shell and device dashboard now exist — the app launches to a dashboard of detected devices, checks a connected keyboard's firmware against the vendor endpoint ("Check for Updates"), and reaches demo mode with no hardware. Two per-device editors are in: opening a **Freestyle Edge RGB** shows its keyboard picture and edits remaps (click a key, press a key), with per-layer switching, the three reset scopes and Save, and the **Savant Elite2** opens a real (read-only) view of its seven pedal inputs; every other device still opens the editor placeholder, because only that one board's visual is authored (issues #39–#42). **Macro editing and the feature dialogs are in** — the Macros tab records macros through keystroke capture, with slots, co-triggers, speed/repeat, budgets and the profile's macro list, and the editor hosts spec 11's Tap and Hold (§11.1), Macro Timing Delays (§11.3), Search Keys (§11.6) and Export (§11.5) panels plus layout/led Import, all firmware-gated where spec 09 §2 says so. Still open from that area: Multimodifiers (§11.2) and Select Macro (§11.4) ride with the Advantage 360 visual (#41), Diagnostics (§11.7) and Troubleshoot (§11.8) are #46, and lighting/settings editing is #16.** The solution lives at `src/KinesisEdit.sln` (Avalonia app `KinesisEdit`, UI-free domain library `KinesisEdit.Core`, xUnit tests `KinesisEdit.Core.Tests` for Core and `KinesisEdit.Tests` for the app layer); see `docs/app/solution-structure.md` for the layout and dependency rules. From the repo root:
...
- `docs/app/` — Agent-first documentation of the new app's modules (see "Documentation rules" below). Currently: `solution-structure.md` (projects, commands, CI), `domain-data.md` (the static domain-data layer in Core), `keyboard-model.md` (the runtime keyboard/macro model), `firmware.md` (version parsing, feature gating, update check), `vdrive.md` (v-Drive discovery, file I/O, eject), `lighting.md` (lighting model + led-file engine), `settings.md` (the settings engine: keyboard/app settings models, parsers, 4MB gate, service), `keystroke-capture.md` (physical keystroke capture for remap/macro recording), `app-shell.md` (the Avalonia app layer: shell navigation, device dashboard, detection loop, notifications, firmware update dialog), `layout-files.md` (the layout-file engine: layout/macro parsers and serializers per dialect, invalid-line tracking), `profiles.md` (profile load/save/eject orchestration for the numbered-profile devices), `keyboard-editor.md` (the keyboard editor: visual geometry, the generic keyboard control, the remap workflow, the macro panel, keystroke routing), `feature-dialogs.md` (the spec 11 panels hosted inline in the editor — Tap and Hold, Macro Timing Delays, Search Keys, Export — plus Import and the shared firmware-gate refusal), and `savant-elite.md` (the Savant Elite2 pedal-file engine: the seven inputs, `pedals.txt` parse/serialize, save merge, display text, and the read-only pedal view in the shell). Add a doc per module as modules are built.
...
4. Keystroke capture feeding that editing: real keypresses with left/right modifiers distinguished, swallowed while recording a remap or macro. Implemented in `KinesisEdit.Core.Input` (see `docs/app/keystroke-capture.md`); the editor owns the single subscription and routes each keystroke to exactly one consumer — an open feature panel, a recording macro, or a listening key.
5. A per-device visual keyboard UI for editing that model. The app shell that hosts it — window, device dashboard, detection loop, notifications — is implemented in `KinesisEdit` (see `docs/app/app-shell.md`); the keyboard editor on top of it — the visual-geometry layer in `KinesisEdit.Core.Geometry.Visual`, one device-agnostic keyboard control, the click-then-press remap workflow and the macro panel — is implemented for the **Freestyle Edge RGB** (see `docs/app/keyboard-editor.md`), with spec 11's feature panels and layout/led import hosted inline over it (see `docs/app/feature-dialogs.md`, backed by `KinesisEdit.Core.Transfer` plus `TapAndHoldPrecheck`/`MacroDelayTokens`/`KeySearchCatalog` in Core), and the **Savant Elite2** has its own read-only pedal view (see `docs/app/savant-elite.md`). Other devices need only their board picture authored (issues #39–#42); the Advantage 360's Multimodifier and Select Macro dialogs ride with #41, diagnostics/troubleshoot are #46, and lighting and settings editing are issue #16.
