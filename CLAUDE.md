...
Its foundation has landed (issue #85): the **design system** — colour/shape/type/motion tokens in `src/KinesisEdit/Themes/`, the shared style layer in `src/KinesisEdit/Styles/`, embedded IBM Plex Sans/Mono, the reduce-motion switch, a Fluent control-alias layer that puts the base controls on the palette — plus the **headless UI test harness** that guards it. Read `docs/app/design-system.md` before writing any view or style; it also documents **how to test UI work in this repo** (headless rendering, frame capture, what each suite guards). Screens are still being rebuilt on it, so parts of the UI remain in the pre-redesign layout — that doc's "Known gaps" section lists which.
