...
- **Next up — a UI redesign.** Everything above was built to mimic the legacy app closely. A design handoff now supersedes that look, and in places its behavior: a full design system, a key inspector in place of the current dialogs, macros named and edited where the key is, lighting previewed on the board itself, and per-device board art for every model. It lives in [`docs/design/`](docs/design/README.md) and is tracked as its own epic.
